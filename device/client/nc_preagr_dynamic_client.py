...
class NCAGRClient(Client):
    ...
    def encoder(self, params, coding):
        model_local = coding.encode_RS(params, self.args.upload_k, self.upload_r.value)
        return model_local
    ...
    def relay(self, send_queue, nc, block_list, iter):
        for key in self.address_dict.keys():
            if key != 'server' and self.status_table[int(key[6:])] == 0:
                ...
                upload_r = str(self.upload_r.value)  # change iter to str
                upload_r = upload_r.zfill(3)  # change iter into 3 digit

                send_byte = b'998' + bytes(iter_str, encoding="utf8") + bytes(upload_r, encoding="utf8") + model_glob_byte
