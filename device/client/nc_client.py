...
class NCClient(Client):
    ...
    def relay(self, send_queue, nc, block_list, iter):
        for key in self.address_dict.keys():
            if key != 'server' and self.status_table[int(key[6:])] == 0:
                ...
                send_byte = b'998' + bytes(iter_str, encoding="utf8") + model_glob_byte
    ...
    def decoder(self, nc, local_iter, log):
        ...
        while True:
            ...
            self.relay(self.send_queue, nc, block_list, local_iter)
    ...
    def upload(self, model_local, log):
        for idx, part in enumerate(model_local):
            ...
            if idx < self.args.upload_k:
                # log.info("Block " + str(idx) + " is upload to queue to server at {0} ".format(time.time()))
                self.send_queue.put((shm_name, 'server'))
                # self.put_send_queue(self.send_queue, (model_local_byte, 'server'))
            elif idx%(self.args.num_users-1) < self.ID:
                # log.info("Block " + str(idx) + " is upload to queue to client" + str(idx%(self.args.num_users-1)) + " at {0} ".format(time.time()))
                self.send_queue.put((shm_name, 'client'+str(idx%(self.args.num_users-1))))
                # self.put_send_queue(self.send_queue, (model_local_byte, 'client'+str(idx%(self.args.num_users-1))))
            elif idx%(self.args.num_users-1) >= self.ID:
                # log.info("Block " + str(idx) + " is upload to queue to client" + str(idx%(self.args.num_users-1)+1) + " at {0} ".format(time.time()))
                self.send_queue.put((shm_name, 'client'+str(idx%(self.args.num_users-1)+1)))
                # self.put_send_queue(self.send_queue, (model_local_byte, 'client'+str(idx%(self.args.num_users-1)+1)))
    ...
    def upload_sender_func(self, iter, key, queue, ack, buffer_queue, log):
        asyncio.run(self.upload_sender(iter, key, queue, ack, buffer_queue, log))
